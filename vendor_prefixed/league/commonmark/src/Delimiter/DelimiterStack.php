...
final class DelimiterStack {
	...
	private function findEarliest(int $stackBottom): ?DelimiterInterface {
		...
		$delimiter   = $this->top;
		$lastChecked = null;
		while ( null !== $delimiter && self::getIndex($delimiter) > $stackBottom ) {
	...
	private function removeDelimitersBetween(DelimiterInterface $opener, DelimiterInterface $closer): void {
		...
		while ( null !== $delimiter && self::getIndex($delimiter) > $openerPosition ) {
	...
	public function deactivateLinkOpeners(): void {
		...
		while ( null !== $opener && $opener->isActive() ) {
			$opener->setActive(\false);
			$opener = $opener->getPrevious();
		}
	}
	...
	public function processDelimiters($stackBottom, DelimiterProcessorCollection $processors): void {
		...
		while ( null !== $closer ) {
			...
			if ( ! $closer->canClose() || null === $delimiterProcessor ) {
			...
			while ( null !== $opener && ( $openerPosition = self::getIndex($opener) ) > $stackBottomPosition && $openerPosition >= ( $openersBottom[ $openersBottomCacheKey ] ?? 0 ) ) {
			...
			if ( ! $openerFound ) {
				...
				if ( \false === $potentialOpenerFound || $delimiterProcessor instanceof CacheableDelimiterProcessorInterface ) {
