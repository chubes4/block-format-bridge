...
final class Environment implements EnvironmentInterface, EnvironmentBuilderInterface, ListenerProviderInterface {
	 ...
	 * @psalm-readonly-allow-private-mutation
	 */
	private array $uninitializedExtensions = array();
	/** @psalm-readonly-allow-private-mutation */
	 ...
	 * @psalm-readonly-allow-private-mutation
	 */
	private array $renderersByClass = array();
	/**
	...
	private function injectEnvironmentAndConfigurationIfNeeded(object $object_value): void {
		if ( $object_value instanceof EnvironmentAwareInterface ) {
			$object_value->setEnvironment($this);
		}
		if ( $object_value instanceof ConfigurationAwareInterface ) {
			$object_value->setConfiguration($this->config->reader());
		}
	}
	/**
	 ...
	 * @param array<string, mixed> $config
	 */
	public static function createCommonMarkEnvironment(array $config = array()): Environment {
	 ...
	 * @param array<string, mixed> $config
	 */
	public static function createGFMEnvironment(array $config = array()): Environment {
		$environment = new self($config);
		$environment->addExtension(new CommonMarkCoreExtension());
		...
		if ( null === $this->slugNormalizer ) {
			...
			if ( $normalizer instanceof UniqueSlugNormalizer ) {
				if ( $this->config->get('slug_normalizer/unique') === UniqueSlugNormalizerInterface::PER_DOCUMENT ) {
					$this->addEventListener(DocumentParsedEvent::class, array( $normalizer, 'clearHistory' ), -1000);
				}
			}
