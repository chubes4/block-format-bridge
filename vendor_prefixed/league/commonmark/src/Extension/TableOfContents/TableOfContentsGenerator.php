...
final class TableOfContentsGenerator implements TableOfContentsGeneratorInterface {
	...
	public function generate(Document $document): ?TableOfContents {
		...
		foreach ( $this->getHeadingLinks($document) as $headingLink ) {
			...
			// Create the new link
			$link     = new Link('#' . $this->fragmentPrefix . $headingLink->getSlug(), StringContainerHelper::getChildText($heading, array( RawMarkupContainerInterface::class )));
		...
		if ( ! $toc->hasChildren() || null === $firstHeading ) {
			return null;
		}
		return $toc;
	}
	private function createToc(Document $document): TableOfContents {
		...
		if ( self::STYLE_BULLET === $this->style ) {
			$listData->type = ListBlock::TYPE_BULLET;
		} elseif ( self::STYLE_ORDERED === $this->style ) {
