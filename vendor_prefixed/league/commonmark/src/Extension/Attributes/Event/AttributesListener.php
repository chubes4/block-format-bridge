...
final class AttributesListener {
	 ...
	 */
	public function __construct(array $allowList = array(), bool $allowUnsafeLinks = \true) {
	...
	private static function findTargetAndDirection($node): array {
		...
		while ( \true ) {
			...
			if ( null !== $previous && ! self::isAttributesNode($previous) ) {
				$target    = $previous;
				$direction = self::DIRECTION_SUFFIX;
				break;
			}
			if ( null !== $next && ! self::isAttributesNode($next) ) {
				$target    = $next;
				$direction = self::DIRECTION_PREFIX;
				break;
			}
		}
		return array( $target, $direction );
	}
	/**
