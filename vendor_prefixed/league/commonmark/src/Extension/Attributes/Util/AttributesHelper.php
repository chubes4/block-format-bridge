...
final class AttributesHelper {
	...
	public static function mergeAttributes($attributes1, $attributes2): array {
		$attributes = array();
		foreach ( array( $attributes1, $attributes2 ) as $arg ) {
	...
	public static function filterAttributes(array $attributes, array $allowList, bool $allowUnsafeLinks): array {
		...
		foreach ( $attributes as $name => $value ) {
			...
			if ( ! $allowUnsafeLinks && ( 'href' === $attrNameLower || 'src' === $attrNameLower ) && \is_string($value) && RegexHelper::isLinkPotentiallyUnsafe($value) ) {
