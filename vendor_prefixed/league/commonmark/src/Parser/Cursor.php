...
class Cursor {
	...
	public function __construct(string $line) {
		...
		$this->line            = $line;
		$this->length          = \mb_strlen($line, 'UTF-8') ? mb_strlen($line, 'UTF-8') : 0;
		$this->isMultibyte     = \strlen !== $this->length($line);
	...
	public function getNextNonSpacePosition(): int {
		if ( null !== $this->nextNonSpaceCache ) {
	...
	public function advanceBy(int $characters, bool $advanceByColumns = \false): void {
		...
		if ( $this->currentPosition >= $this->length || 0 === $characters ) {
			return;
		}
		// Optimization to avoid tab handling logic if we have no tabs
		if ( \false === $this->lastTabPosition || $this->currentPosition > $this->lastTabPosition ) {
	...
	public function saveState(): CursorState {
		return new CursorState(array( $this->currentPosition, $this->previousPosition, $this->nextNonSpaceCache, $this->indent, $this->column, $this->partiallyConsumedTab ));
	}
	/**
