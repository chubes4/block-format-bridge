...
final class RegexHelper {
	 ...
	 * @psalm-pure
	 */
	public static function matchAt(string $regex, string $string_value, int $offset = 0): ?int {
		$matches      = array();
		$string_value = \mb_substr($string_value, $offset, null, 'UTF-8');
		if ( ! \preg_match($regex, $string_value, $matches, \PREG_OFFSET_CAPTURE) ) {
			return null;
		}
		// PREG_OFFSET_CAPTURE always returns the byte offset, not the char offset, which is annoying
		$charPos = \mb_strlen(\mb_strcut($string_value, 0, $matches[0][1], 'UTF-8'), 'UTF-8');
	 ...
	 * @psalm-pure
	 */
	public static function unescape(string $string_value): string {
		$allEscapedChar = '/\\\\(' . self::PARTIAL_ESCAPABLE . ')/';
		$escaped        = \preg_replace($allEscapedChar, '$1', $string_value);
