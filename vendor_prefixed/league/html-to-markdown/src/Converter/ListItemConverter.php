	...
	public function convert(ElementInterface $element): string {
		...
		if ( 'ul' === $listType ) {
			...
			if ( ! isset($this->listItemStyle) ) {
				$this->listItemStyle = $listItemStyleAlternate ? $listItemStyleAlternate : $listItemStyle;
			}
			if ( $listItemStyleAlternate && 0 === $level && $element->getSiblingPosition() === 1 ) {
		...
		if ( 'ol' === $listType && ( $parent = $element->getParent() ) && $start = \intval($parent->getAttribute('start')) ) {
