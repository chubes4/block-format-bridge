...
final class Configuration implements ConfigurationBuilderInterface, ConfigurationInterface {
	 ...
	 */
	private array $configSchemas = array();
	/** @psalm-allow-private-mutation */
	 ...
	 */
	public function __construct(array $baseSchemas = array()) {
	...
	private function build(string $topLevelKey): void {
		...
		try {
			$userData = array( $topLevelKey => $this->userConfig->get($topLevelKey) );
		} catch ( DataException $ex ) {
			$userData = array();
		}
		try {
			...
			$processed = $processor->process(Expect::structure(array( $topLevelKey => $schema )), $userData);
