...
final class Expect {
	...
	public static function from(object $object_value, array $items = array()): Structure {
		$ro    = new \ReflectionObject($object_value);
		...
		foreach ( $props as $prop ) {
			...
			if ( ! isset($items[ $name ]) ) {
				...
				$item = new Type($type);
				if ( $prop instanceof \ReflectionProperty ? $prop->isInitialized($object_value) : $prop->isOptional() ) {
					$def = $prop instanceof \ReflectionProperty ? $prop->getValue($object_value) : $prop->getDefaultValue();
					if ( is_object($def) ) {
						$item = self::from($def);
					} elseif ( null === $def && ! Nette\Utils\Validators::is(null, $type) ) {
						$item->required();
					} else {
						$item->default($def);
					}
	 ...
	 * @param  mixed[]  $shape
	 */
	public static function array(?array $shape = array()): Structure|Type {
		$shape ??= array();
