...
trait Base {
	...
	private array $transforms   = array();
	...
	public function assert(callable $handler, ?string $description = null): self {
		...
		return $this->transform(function ($value, Context $context) use ($handler, $description, $expected) {
			...
			$context->addError('Failed assertion ' . ( $description ? "'%assertion%'" : '%assertion%' ) . ' for %label% %path% with value %value%.', Nette\Schema\Message::FailedAssertion, array(
				'value'     => $value,
				'assertion' => $expected,
			));
		});
