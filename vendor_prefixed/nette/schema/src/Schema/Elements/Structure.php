...
final class Structure implements Schema {
	...
	public function merge(mixed $value, mixed $base): mixed {
		...
		if ( is_array($value) && is_array($base) ) {
			$index = null === $this->otherItems ? null : 0;
			foreach ( $value as $key => $val ) {
				if ( $key === $index ) {
					$base[] = $val;
					++$index;
		...
		if ( $extraKeys = array_keys(array_diff_key($value, $items)) ) {
			if ( $this->otherItems ) {
				...
				foreach ( $extraKeys as $key ) {
					...
					$context->addError('Unexpected item %path%' . ( $hint ? ", did you mean '%hint%'?" : '.' ), Nette\Schema\Message::UnexpectedItem, array( 'hint' => $hint ))->path[] = $key;
				}
			}
		}
	...
	public function completeDefault(Context $context): mixed {
		return $this->required ? $this->complete(array(), $context) : null;
	}
}
