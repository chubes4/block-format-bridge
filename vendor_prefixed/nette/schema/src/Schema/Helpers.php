...
final class Helpers {
	...
	public static function merge(mixed $value, mixed $base): mixed {
		...
		if ( is_array($value) && is_array($base) ) {
			...
			return $base;
		} elseif ( null === $value && is_array($base) ) {
			return $base;
		} else {
			return $value;
		}
	...
	public static function validateType(mixed $value, string $expected, Context $context): void {
		if ( ! Nette\Utils\Validators::is($value, $expected) ) {
			...
			$context->addError('The %label% %path% expects to be %expected%, %value% given.', Message::TypeMismatch, array(
				'value'    => $value,
				'expected' => $expected,
			));
		}
	...
	public static function validateRange(mixed $value, array $range, Context $context, string $types = ''): void {
		if ( is_array($value) || is_string($value) ) {
			[$length, $label] = is_array($value) ? array( count($value), 'items' ) : ( in_array('unicode', explode('|', $types), strict: \true) ? array( Nette\Utils\Strings::length($value), 'characters' ) : array( strlen($value), 'bytes' ) );
			if ( ! self::isInRange($length, $range) ) {
				$context->addError("The length of %label% %path% expects to be in range %expected%, %length% {$label} given.", Message::LengthOutOfRange, array(
					'value'    => $value,
					'length'   => $length,
					'expected' => implode('..', $range),
				));
			}
		} elseif ( ( is_int($value) || is_float($value) ) && ! self::isInRange($value, $range) ) {
			$context->addError('The %label% %path% expects to be in range %expected%, %value% given.', Message::ValueOutOfRange, array(
				'value'    => $value,
				'expected' => implode('..', $range),
			));
		}
	...
	public static function isInRange(mixed $value, array $range): bool {
		return ( null === $range[0] || $value >= $range[0] ) && ( null === $range[1] || $value <= $range[1] );
	}
	public static function validatePattern(string $value, string $pattern, Context $context): void {
		if ( ! preg_match("\x01^(?:{$pattern})\$\x01Du", $value) ) {
			$context->addError("The %label% %path% expects to match pattern '%pattern%', %value% given.", Message::PatternMismatch, array(
				'value'   => $value,
				'pattern' => $pattern,
			));
		}
