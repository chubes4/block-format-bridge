...
class Validators {
	...
	private const BuiltinTypes = array(
		'string'   => 1,
		'int'      => 1,
		'float'    => 1,
		'bool'     => 1,
	...
	protected static $validators = array(
		...
		'list'       => array( Arrays::class, 'isList' ),
		'mixed'      => array( self::class, 'isMixed' ),
		'none'       => array( self::class, 'isNone' ),
		'number'     => array( self::class, 'isNumber' ),
		'numeric'    => array( self::class, 'isNumeric' ),
		'numericint' => array( self::class, 'isNumericInt' ),
		// string patterns
		'alnum'      => 'ctype_alnum',
		'alpha'      => 'ctype_alpha',
		'digit'      => 'ctype_digit',
		...
		'pattern'    => null,
		'space'      => 'ctype_space',
		'unicode'    => array( self::class, 'isUnicode' ),
		'upper'      => 'ctype_upper',
		'xdigit'     => 'ctype_xdigit',
		// syntax validation
		'email'      => array( self::class, 'isEmail' ),
		'identifier' => array( self::class, 'isPhpIdentifier' ),
		'uri'        => array( self::class, 'isUri' ),
		'url'        => array( self::class, 'isUrl' ),
		// environment validation
		'class'      => 'class_exists',
		'interface'  => 'interface_exists',
		'directory'  => 'is_dir',
		'file'       => 'is_file',
		'type'       => array( self::class, 'isType' ),
	);
	/** @var array<string, callable(mixed): int> */
	protected static $counters = array(
		'string'  => 'strlen',
		'unicode' => array( Strings::class, 'length' ),
		'array'   => 'count',
		'list'    => 'count',
		'alnum'   => 'strlen',
		'alpha'   => 'strlen',
	 ...
	 * @throws AssertionException
	 */
	public static function assertField(array $array_value, int|string $key, ?string $expected = null, string $label = "item '%' in array"): void {
		if ( ! array_key_exists($key, $array_value) ) {
			throw new AssertionException('Missing ' . str_replace('%', (string) $key, $label) . '.');
		} elseif ( $expected ) {
			static::assert($array_value[ $key ], $expected, str_replace('%', (string) $key, $label));
		}
	}
	/**
	 * Verifies that the value is of expected types separated by pipe.
	...
	public static function isInRange(mixed $value, array $range): bool {
		if ( null === $value || ! ( isset($range[0]) || isset($range[1]) ) ) {
			return \false;
		}
