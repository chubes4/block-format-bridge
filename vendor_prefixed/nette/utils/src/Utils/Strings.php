...
class Strings {
	...
	public static function chr(int $code): string {
		...
		return \false === $res ? throw new Nette\ShouldNotHappenException() : $res;
	}
	/**
	...
	public static function substring(string $s, int $start, ?int $length = null): string {
		...
		return \false === $res ? throw new Nette\InvalidStateException('iconv_substr() failed.') : $res;
	}
	/**
	...
	public static function normalize(string $s): string {
		...
		$s = self::pcre('preg_replace', array( '#[\x00-\x08\x0B-\x1F\x7F-\x9F]+#u', '', $s ));
		// right trim
		$s = self::pcre('preg_replace', array( '#[\t ]+$#m', '', $s ));
	...
	public static function toAscii(string $s): string {
		...
		$s = self::pcre('preg_replace', array( '#[^\x09\x0A\x0D\x20-\x7E\xA0-\x{2FF}\x{370}-\x{10FFFF}]#u', '', $s ));
		...
		if ( 'glibc' === $iconv ) {
			...
			$s = str_replace(array( '?', "\x01" ), array( '', '?' ), $s);
			// remove garbage and restore ? characters
		} elseif ( 'libiconv' === $iconv ) {
			$s = iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $s);
			if ( \false === $s ) {
				throw new Nette\InvalidStateException('iconv() failed.');
			}
			...
			// null or 'unknown' (#216)
			$s = self::pcre('preg_replace', array( '#[^\x00-\x7F]++#', '', $s ));
	...
	public static function webalize(string $s, ?string $charlist = null, bool $lower = \true): string {
		...
		$s = self::pcre('preg_replace', array( '#[^a-z0-9' . ( null !== $charlist ? preg_quote($charlist, '#') : '' ) . ']+#i', '-', $s ));
		$s = trim($s, '-');
		return $s;
	}
	/**
	...
	public static function reverse(string $s): string {
		...
		return \false === $tmp ? throw new Nette\InvalidStateException('iconv() failed.') : (string) iconv('UTF-32LE', 'UTF-8', strrev($tmp));
	}
	/**
	...
	public static function before(string $haystack, string $needle, int $nth = 1): ?string {
		...
		return null === $pos ? null : substr($haystack, 0, $pos);
	}
	/**
	...
	public static function after(string $haystack, string $needle, int $nth = 1): ?string {
		$pos = self::pos($haystack, $needle, $nth);
		return null === $pos ? null : substr($haystack, $pos + strlen($needle));
	}
	/**
	...
	public static function indexOf(string $haystack, string $needle, int $nth = 1): ?int {
		$pos = self::pos($haystack, $needle, $nth);
		return null === $pos ? null : self::length(substr($haystack, 0, $pos));
	}
	/**
	 * Returns position in characters of $nth occurence of $needle in $haystack or null if the needle was not found.
	 */
	...
	public static function split(
		...
		$pattern .= $utf8 ? 'u' : '';
		$m        = self::pcre('preg_split', array( $pattern, $subject, $limit, $flags | PREG_SPLIT_DELIM_CAPTURE ));
		return $utf8 && $captureOffset ? self::bytesToChars($subject, array( $m ))[0] : $m;
	}
	/**
	...
	public static function match(
		...
		if ( $offset > strlen($subject) ) {
			return null;
		} elseif ( ! self::pcre('preg_match', array( $pattern, $subject, &$m, $flags, $offset )) ) {
			return null;
		} elseif ( $utf8 && $captureOffset ) {
			return self::bytesToChars($subject, array( $m ))[0];
		} else {
			return $m;
		}
	}
	...
	public static function matchAll(
		...
		if ( $lazy ) {
			...
			return ( function () use ($utf8, $captureOffset, $flags, $subject, $pattern, $offset) {
				$counter = 0;
				$m       = array();
				while ( $offset <= strlen($subject) - ( $counter ? 1 : 0 ) && self::pcre('preg_match', array( $pattern, $subject, &$m, $flags, $offset )) ) {
					/** @var list<array{string, int}> $m */
					$offset = $m[0][1] + max(1, strlen($m[0][0]));
					if ( ! $captureOffset ) {
						$m = array_map(fn($item) => $item[0], $m);
					} elseif ( $utf8 ) {
						$m = self::bytesToChars($subject, array( $m ))[0];
					}
					yield $counter++ => $m;
				}
			} )();
		...
		$m     = array();
		self::pcre('preg_match_all', array( $pattern, $subject, &$m, $flags & PREG_PATTERN_ORDER ? $flags : $flags | PREG_SET_ORDER, $offset ));
	...
	public static function replace(
		...
		if ( is_object($replacement) || is_array($replacement) ) {
			...
			if ( $utf8 ) {
				...
				if ( $captureOffset ) {
					$replacement = fn($m) => $replacement(self::bytesToChars($subject, array( $m ))[0]);
				}
			}
			return self::pcre('preg_replace_callback', array( $pattern, $replacement, $subject, $limit, 0, $flags ));
	...
	public static function pcre(string $func, array $args): mixed {
		...
		if ( ( $code = preg_last_error() ) && ( null === $res || ! in_array($func, array( 'preg_filter', 'preg_replace_callback', 'preg_replace' ), strict: \true) ) ) {
