...
class Image {
	...
	private static function invokeSafe(string $func, string $arg, string $message, string $callee): static {
		$errors = array();
		$res    = Callback::invokeSafe($func, array( $arg ), function (string $message) use (&$errors): void {
			$errors[] = $message;
		});
		if ( ! $res ) {
	...
	public static function extensionToType(string $extension): int {
		$extensions = array_flip(self::Formats) + array( 'jpg' => ImageType::JPEG );
	...
	public static function calculateSize(int $srcWidth, int $srcHeight, int|string|null $newWidth, int|string|null $newHeight, int $mode = self::OrSmaller): array {
		...
		if ( $mode & self::Stretch ) {
			...
			if ( $mode & self::OrBigger ) {
				$scale = array( max($scale ? $scale : array( 1 )) );
			}
			if ( $mode & self::ShrinkOnly ) {
				$scale[] = 1;
			}
			$scale     = min($scale ? $scale : array( 1 ));
	 ...
	 */
	public static function calculateTextBox(string $text, string $fontFile, float $size, float $angle = 0, array $options = array()): array {
		self::ensureExtension();
		$box = imagettfbbox($size, $angle, $fontFile, $text, $options);
		return array(
			'left'   => $minX = min(array( $box[0], $box[2], $box[4], $box[6] )),
			'top'    => $minY  = min(array( $box[1], $box[3], $box[5], $box[7] )),
			'width'  => max(array( $box[0], $box[2], $box[4], $box[6] )) - $minX + 1,
			'height' => max(array( $box[1], $box[3], $box[5], $box[7] )) - $minY + 1,
		);
	}
	/**
	 * Draw a rectangle.
	...
	private function output(int $type, ?int $quality, ?string $file = null): void {
		[$defQuality, $min, $max] = match ( $type ) {
			ImageType::JPEG => array( 85, 0, 100 ),
			ImageType::PNG => array( 9, 0, 9 ),
			ImageType::GIF => array( null, null, null ),
			ImageType::WEBP => array( 80, 0, 100 ),
			ImageType::AVIF => array( 30, 0, 100 ),
			ImageType::BMP => array( null, null, null ),
			default => throw new Nette\InvalidArgumentException("Unsupported image type '{$type}'."),
		};
		$args = array( $this->image, $file );
		if ( null !== $defQuality ) {
			$args[] = null === $quality ? $defQuality : max($min, min($max, $quality));
		}
	...
	public function __clone() {
		...
		$this->setImageResource(imagecreatefromstring(ob_get_clean()) ? imagecreatefromstring(ob_get_clean()) : throw new Nette\ShouldNotHappenException());
	}
	...
	public function resolveColor(ImageColor|array $color): int {
		...
		return imagecolorallocatealpha($this->image, ...$color) ? imagecolorallocatealpha($this->image, ...$color) : imagecolorresolvealpha($this->image, ...$color);
	}
	/** @param  ImageColor|array{red: int, green: int, blue: int, alpha?: int}  $color */
