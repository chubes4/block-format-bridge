...
class Finder implements \IteratorAggregate {
	...
	private array $find = array();
	/** @var string[] */
	private array $in = array();
	/** @var array<\Closure(FileInfo): bool> */
	private array $filters = array();
	/** @var array<\Closure(FileInfo): bool> */
	private array $descentFilters = array();
	/** @var array<string|self> */
	private array $appends   = array();
	 ...
	 */
	public static function find(string|array $masks = array( '*' )): static {
	 ...
	 * @param  string|list<string>  $masks
	 */
	public static function findFiles(string|array $masks = array( '*' )): static {
		$masks = is_array($masks) ? $masks : func_get_args();
		// compatibility with variadic
	 ...
	 * @param  string|list<string>  $masks
	 */
	public static function findDirectories(string|array $masks = array( '*' )): static {
		$masks = is_array($masks) ? $masks : func_get_args();
		// compatibility with variadic
	 ...
	 * @param  string|list<string>  $masks
	 */
	public function files(string|array $masks = array( '*' )): static {
	 ...
	 * @param  string|list<string>  $masks
	 */
	public function directories(string|array $masks = array( '*' )): static {
	 ...
	 * @return \Generator<string, FileInfo>
	 */
	private function traverseDir(string $dir, array $searches, array $subdirs = array()): \Generator {
		...
		foreach ( $files as $file ) {
			...
			if ( $this->childFirst && $subSearch ) {
				yield from $this->traverseDir($pathName, $subSearch, array_merge($subdirs, array( $file->getBasename() )));
			}
	...
	private function buildPlan(): array {
		...
		foreach ( $this->find as [$mask, $mode] ) {
			...
			if ( FileSystem::isAbsolute($mask) ) {
				...
				foreach ( $this->in ? $this->in : array( '.' ) as $in ) {
					$in = strtr($in, array(
						'[' => '[[]',
						']' => '[]]',
					));
			...
			foreach ( $splits as [$base, $rest, $recursive] ) {
				$base = '' === $base ? '.' : $base;
				$dirs = $dirCache[ $base ] ??= strpbrk($base, '*?[') ? glob($base, GLOB_NOSORT | GLOB_ONLYDIR | GLOB_NOESCAPE) : array(
					strtr($base, array(
						'[[]' => '[',
						'[]]' => ']',
					)),
				...
				$search = (object) array(
					'pattern'   => $this->buildPattern($rest),
					'mode'      => $mode,
					'recursive' => $recursive,
				);
				foreach ( $dirs as $dir ) {
	...
	private static function splitRecursivePart(string $path): array {
		...
		return isset($parts[1]) ? array( $parts[0], $parts[1] . substr($path, $a + 1), \true ) : array( $parts[0], substr($path, $a + 1), \false );
	}
	/**
