...
final class FileSystem {
	...
	public static function copy(string $origin, string $target, bool $overwrite = \true): void {
		if ( stream_is_local($origin) && ! file_exists($origin) ) {
			...
			if ( @stream_copy_to_stream(self::open($origin, 'rb'), self::open($target, 'wb')) === \false ) {
	...
	public static function readLines(string $file, bool $stripNewLines = \true): \Generator {
		return ( function ($f) use ($file, $stripNewLines) {
			$counter = 0;
			do {
				$line = Callback::invokeSafe('fgets', array( $f ), fn($error) => throw new Nette\IOException(sprintf("Unable to read file '%s'. %s", self::normalizePath($file), $error)));
				if ( \false === $line ) {
					fclose($f);
					break;
				}
	...
	public static function normalizePath(string $path): string {
		$parts = '' === $path ? array() : preg_split('~[/\\\\]+~', $path);
		$res   = array();
		...
		return $res === array( '' ) ? DIRECTORY_SEPARATOR : implode(DIRECTORY_SEPARATOR, $res);
	}
	/**
