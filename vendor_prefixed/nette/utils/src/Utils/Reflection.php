...
final class Reflection {
	...
	public static function getMethodDeclaringMethod(\ReflectionMethod $method): \ReflectionMethod {
		...
		$hash = array( $method->getFileName(), $method->getStartLine(), $method->getEndLine() );
		if ( ( $alias = $decl->getTraitAliases()[ $method->name ] ?? null ) && ( $m = new \ReflectionMethod(...explode('::', $alias, 2)) ) && $hash === array( $m->getFileName(), $m->getStartLine(), $m->getEndLine() ) ) {
			return self::getMethodDeclaringMethod($m);
		}
		foreach ( $decl->getTraits() as $trait ) {
			if ( $trait->hasMethod($method->name) && ( $m = $trait->getMethod($method->name) ) && $hash === array( $m->getFileName(), $m->getStartLine(), $m->getEndLine() ) ) {
				return self::getMethodDeclaringMethod($m);
			}
		}
		return $method;
	...
	public static function expandClassName(string $name, \ReflectionClass $context): string {
		...
		if ( empty($name) ) {
			...
			return $lower;
		} elseif ( 'self' === $lower || 'static' === $lower ) {
			return $context->name;
		} elseif ( 'parent' === $lower ) {
	 ...
	 */
	public static function getUseStatements(\ReflectionClass $class_name): array {
		global $wp_filesystem;
		if ( $class_name->isAnonymous() ) {
			throw new Nette\NotImplementedException('Anonymous classes are not supported.');
		}
		static $cache = array();
		if ( ! isset($cache[ $name = $class_name->name ]) ) {
	...
	private static function parseUseStatements(string $code, ?string $forClass = null): array {
		...
		$classLevel = $level = 0;
		$res        = $uses = array();
		$nameTokens = array( T_STRING, T_NS_SEPARATOR, T_NAME_QUALIFIED, T_NAME_FULLY_QUALIFIED );
	...
	private static function fetch(array &$tokens, string|int|array $take): ?string {
		...
		while ( $token = current($tokens) ) {
			if ( $token->is($take) ) {
				$res .= $token->text;
			} elseif ( ! $token->is(array( T_DOC_COMMENT, T_WHITESPACE, T_COMMENT )) ) {
				break;
			}
			next($tokens);
		}
